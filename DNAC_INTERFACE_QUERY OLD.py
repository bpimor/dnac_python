...
from DNAC_CONNECTION import BASE_URL, DNAC_INTERFACES_OLD, DNAC_INT_COUNT
...
def is_a_switch (devicetype, deviceuuid, deviceid,): 
    if "Switch" in devicetype:
        ...
        response = requests.get(BASE_URL + DNAC_INTERFACES_OLD.format(deviceId = deviceid), headers = headers, verify=False)
        ...
        for interfaces in range(interface_count):
            # print(response.json()['response'][interfaces]['adminStatus'])
            interface_status = response.json()['response'][interfaces]['value']['adminStatus']
            interface_name = response.json()['response'][interfaces]['value']['name']
            interface_type = response.json()['response'][interfaces]['value']['interfaceType']
