# File dependencies: DNAC_CONNECTION.py, DNAC_INTERFACE_COUNT_PER_DEVICE.py, GET_DNAC_TOKEN.py, DNACL_CLI_SH_TRANSCEIVER.py
...
from DNAC_CONNECTION import BASE_URL, DNAC_INTERFACES, DNAC_INT_COUNT
...
def interface_count_function(deviceid): 
...
def is_a_switch (devicetype, deviceuuid, deviceid,): 
    if "Switch" in devicetype:
        ...
        response = requests.get(BASE_URL + DNAC_INTERFACES.format(deviceId = deviceid), headers = headers, verify=False)
        # Get the total count of interfaces on the switch for creating a range
        interface_count = interface_count_function(deviceid)
        # Goes through all the interfaces of the switch
        for interfaces in range(interface_count):
            interface_status = response.json()['response'][interfaces]['adminStatus']
            interface_name = response.json()['response'][interfaces]['portName']
            interface_type = response.json()['response'][interfaces]['interfaceType']
