...
text_file = open('sh_license.txt', 'w')
for device in devices_list:
    ...
    if "Switch" in device_type:
        ...
        payload = '{ "commands": [ "show license status" ], "deviceUuids": ["' + device_uuid + '"] }'
        ...
        try:
            print(final_result.json()[0]['commandResponses']['SUCCESS']['show license status'])
            text_file.writelines(final_result.json()[0]['commandResponses']['SUCCESS']['show license status']+"\n")
        except: 
